...
class Tray:
    ...
    def _menu_properties(self, item_id: int) -> dict[str, Any]:
        ...
        if item_id == QUIT_ID:
            ...
            # by smbd out of smbpal.conf, and the connections stay mounted
            # because systemd's automount units hold them, not us. A label
            # promising to quit SMBPal would be promising something no tray
            # icon can do — and something no per-user icon should do to the
            # other people logged in. What it closes is the icon, so that is
            # the whole of what it says.
            return {
                "label": GLib.Variant("s", "Close"),
